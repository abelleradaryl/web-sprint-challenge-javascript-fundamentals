...
// 'nestedFunction' can access the variable internal because it is completely contained within its parent function 'myFunction'.
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    const lowerCaseArr = arr.map(element => element.animal_name.toLowerCase());
     return lowerCaseArr;
    }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const newArr = arr.filter(element => element.population < 5);
    return newArr;
  }
  ...
  */
  ...
  function USApop(arr){
    const totalPop = arr.reduce((accumulator, element) => {
